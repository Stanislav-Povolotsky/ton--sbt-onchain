...
describe('SbtItem (in-collection mode)', () => {
    ...
    let EDITOR_ADDRESS: Address | null;
    ...
    it('should not allow to edit', async () => {
        ...
        let res1 = await token.sendEditContent(authority_wallet.getSender(), { content: encodeOnChainContent(configMetadataEdit) });
        // should fail if sender is not editor (owner)
        expect(res1.transactions).toHaveTransaction({
            from: authority_wallet.address,
            to: token.address,
            success: false,
            exitCode: 0xffff, // Edit operation is not supported
        });

        let res2 = await token.sendEditContent(deployer.getSender(), { content: encodeOnChainContent(configMetadataEdit) });
